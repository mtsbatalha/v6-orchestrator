...
def rclone_download(remote, remote_path, local_dir, logger):
    ...
    if remote.startswith("/"):
        rclone_src = f"{remote}/{remote_path}/"
    else:
        rclone_src = f"{remote}:{remote_path}/"
    cmd = f"rclone copy {repr(rclone_src)} {repr(local_dir + '/')} -P --transfers 2 --checkers 4 --timeout 1h --bwlimit '05:00,off 14:00,60M'"
...
def rclone_upload(local_dir, remote, remote_path, logger):
    ...
    if remote.startswith("/"):
        rclone_dst = f"{remote}/{remote_path}/"
    else:
        rclone_dst = f"{remote}:{remote_path}/"
    cmd = f"rclone copy {repr(local_dir + '/')} {repr(rclone_dst)} -P --transfers 2 --tpslimit 4 --timeout 2h --retries 3 --bwlimit '05:00,off 14:00,60M'"
    rc, out, err = run(cmd, timeout=86400)
    if rc != 0:
        logger.error(f"  Upload failed: {err[:300]}")
        return False
    logger.info(f"  Upload OK: {rclone_dst}")
    return True
...
def process_work_order(work_order, worker_cfg, logger):
    ...
    try:
        ...
        if existing_videos:
            ...
            cmd = f"rclone copy {repr(rclone_full_path)} {repr(item_temp + '/')} -P --transfers 4 --checkers 8 --timeout 1h"
            rc, out, err = run(cmd, timeout=86400)
            if rc != 0:
                logger.error(f"  Download failed: {err[:300]}")
...
def main():
    ...
    if args.mode == "process-work-order":
        ...
        # Worker leaves work_order.json for coordinator busy-check; collect_results cleans up
